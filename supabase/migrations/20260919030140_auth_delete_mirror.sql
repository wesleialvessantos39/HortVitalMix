...
CREATE FUNCTION public.trg_fn_auth_user_deleted()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
BEGIN
  UPDATE public.app_users
  SET status='suspended',block_reason='auth_user_deleted',blocked_at=clock_timestamp(),authorization_revision=authorization_revision+1
  WHERE id=OLD.id;
  RETURN OLD;
END;
$$;
