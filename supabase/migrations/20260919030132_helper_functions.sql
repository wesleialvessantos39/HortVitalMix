...
$$;
CREATE FUNCTION public.is_platform_super_admin()
RETURNS BOOLEAN LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$SELECT public.has_role('platform_super_admin');$$;
CREATE FUNCTION public.is_any_platform_admin()
RETURNS BOOLEAN LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$SELECT public.has_role('platform_super_admin') OR public.has_role('platform_admin');$$;
CREATE FUNCTION public.current_person_id()
RETURNS UUID LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$
  SELECT p.id FROM public.app_people p JOIN public.app_users u ON u.id=p.user_id
  WHERE p.user_id=auth.uid() AND u.status='active' LIMIT 1;
$$;
CREATE FUNCTION public.hash_ip(ip_text TEXT)
RETURNS CHAR(64) LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path=public,extensions AS $$
DECLARE pepper TEXT:=current_setting('app.ip_pepper',true);
BEGIN
  IF pepper IS NULL OR length(pepper)<16 THEN RAISE EXCEPTION 'AUDIT_PEPPER_NOT_CONFIGURED'; END IF;
  RETURN encode(extensions.digest(coalesce(ip_text,'')||pepper,'sha256'),'hex')::CHAR(64);
END;
$$;
