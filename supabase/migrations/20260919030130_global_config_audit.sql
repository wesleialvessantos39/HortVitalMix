...
CREATE TABLE public.app_audit_events(
  ...
  actor_id UUID,
  actor_role VARCHAR(64) NOT NULL DEFAULT 'anonymous',
  action VARCHAR(128) NOT NULL CHECK(action ~ '^[a-z][a-z0-9._]{2,127}$'),
  target_entity VARCHAR(64) NOT NULL,
  target_id UUID,
  payload_before JSONB,
  payload_after JSONB,
  client_ip_hash CHAR(64) NOT NULL CHECK(client_ip_hash ~ '^[0-9a-f]{64}$'),
...
ALTER TABLE public.app_audit_events FORCE ROW LEVEL SECURITY;
...
CREATE FUNCTION public.trg_fn_prevent_audit_tampering()
