...
CREATE VIEW public.v_rls_audit WITH(security_invoker=true) AS
SELECT c.relname AS table_name,c.relrowsecurity AS rls_enabled,c.relforcerowsecurity AS rls_forced,count(p.polname) AS policy_count
FROM pg_class c
LEFT JOIN pg_policy p ON p.polrelid=c.oid
WHERE c.relnamespace='public'::regnamespace AND c.relkind='r' AND c.relname LIKE 'app\_%' ESCAPE '\'
GROUP BY c.relname,c.relrowsecurity,c.relforcerowsecurity;
REVOKE ALL ON public.v_rls_audit FROM PUBLIC,anon,authenticated;
GRANT SELECT ON public.v_rls_audit TO service_role;
...
USING(bucket_id='documents' AND owner_id=(SELECT auth.uid())::text AND (storage.foldername(name))[1]=public.current_person_id()::text);
CREATE POLICY documents_owner_insert ON storage.objects
FOR INSERT TO authenticated
WITH CHECK(bucket_id='documents' AND owner_id=(SELECT auth.uid())::text AND (storage.foldername(name))[1]=public.current_person_id()::text);
